...
class BVC_NAV:
    ...
    def __init__(self,Path,m_tau,steer_thres,telen_neurons,ang_bin,corals_pos,Goal_pos,agent_init_pos,df,eps,free_turn_bins,
                 max_bins_turn,seed,sim_time,sim_num,plt,vid):
        ...
        # self.GoalDirection=float(random.randrange(-10,10))/10 # random direction -1:0.1:1

        ####################### below should be replaced with automatic wall avoidance!#########
        x=np.sign(agent_init_pos[0])
        y=np.sign(agent_init_pos[1])
        if (x < 0 and y < 0):
            q=3
        elif (x < 0 and y > 0):
            q=2
            ...
            q=4
        self.GoalDirection = float(random.randrange(-15+5*q, -10+5*q)) / 10  # random direction -1:0.1:1
        ######################################################################################
    ...
    def buildModel(self):
        ...
        def _get_potential_velocity(t, bvns_data):
            ...
            dist_0 = dist_per_angle[0]- fish_size/2 - self.coral_size #avoid f2f collision
            ...
            if 1:# try to go desired direction
                ...
                # if this fails, maximize distance in front:
                    ...
                    # turn_angle_ind = np.argmax(
                    #     dist_array_wide_front) - n_bins_max_turn  # ind in [-n/2,n/2]
                    # # use the bin as direction, but make turn smooth (size/10)
                    # turn_angle_CCW = turn_angle_ind * ang_bin_size  # actual angel to turn CCW
                ...
                if np.mod(t,1)<self.dt:
                    self.GoalDirection = calc_angle(t, v) #set new direction to the current (only for this model!)
    ...
    def Start(self):
        ...
        while t < self.StopT+0.1:
            ...
            self.sim.run_steps(steps=1, progress_bar=False)  # single simulation step
            ...
            if self.plot_config:
                if np.mod(step_counter,100)==0: #plot every other 100 steps (100ms):
                    ...
                    plt.xticks([])
                    plt.yticks([])
                    # plt.title('BVC_Nav noMemory  t = ' + str(round(t,2)) + ' [s]')
                    plt.title('ref t = ' + str(round(t,2)) + ' [s], GD= ' + str(self.GoalDirection) )
        ...
        if t >= self.StopT and stopflag==True:
