...
class BVC_NAV:
    ...
    def __init__(self,Path,memorize_corals,m_tau,steer_thres,telen_neurons,ang_bin,corals_pos,Goal_pos,agent_init_pos,df,eps,free_turn_bins,max_bins_turn,seed,sim_time,sim_num,plt,vid):
        ...
        self.HP=self.init_pos[1] # initial HP
    ...
    def buildModel(self):
        ...
        def memorized_direction(t):
            ...
            # if t<t_start_memory: #at the beginning don't memorize anything
            #     self.idx_nearby_Corals=[0] #default
            ...
            if t<t_start_memory: #initial default values until LIDAR read is implemented (not used, only avoiding bugs)
            ...
            # function for goal direction by HP as control
            def GoalHP(t):
                x=self.HP-Goal_pos[1]
                if t < .1:  # at the beginning, keep only general direction to goal
                    initial_x_dir_2goal = np.sign(
                        Goal_pos[0] - self.init_pos[0])  # when reaching to Goal's HP, left or right?
                    self.goal_x_direction = np.sign(
                        initial_x_dir_2goal - 1)  # this shifts above sign=-1 to -1 (i.e -pi) and sign=1 to 0 (i.e. 0)
                    GoalDirectionbyHP = .5 * np.sign(x)  # initial value
                elif abs(x) < 4*self.eps:  # if HP is as Goal's HP:
                    GoalDirectionbyHP = self.goal_x_direction
                else:
                    GoalDirectionbyHP = float(.5 * np.sign(x) * self.x_out) #.5 * np.sign(x)  # pi/2 if goal is shallower, -pi/2 if deeper
                return GoalDirectionbyHP
            ...
            def calc_similarity(t, x):
                similarity_vector = spa.similarity(x[:D], Ang2Goal_vocab)
                if np.max(similarity_vector) > 0.25:  # set threshold for recognition
                    ang_str = Ang2Goal_vocab.keys[np.argmax(similarity_vector)]
                    def_hd = ang_array[int(ang_str[1:3]) - ang_bin_shift]
                else:  #use HP as signal
                    def_hd = x[-1]
            ...
            with model:
                ...
                #control : direction by HP
                GoalDirectionByHP=nengo.Node(GoalHP)
                ...
                output_node = nengo.Node(calc_similarity, size_in=D+1)
                nengo.Connection(model.answer.output, output_node[:D], synapse=tau) #connect the vector to check
                nengo.Connection(GoalDirectionByHP,output_node[-1], synapse=tau) #connect the HP direction
        ...
        with self.model:
            ...
            HP = nengo.Node(getHP,size_in=1, size_out=1)  # gets the difference between goal and agent HP
            nengo.Connection(nHP, HP)
        ...
        while t < self.StopT+0.1:
            ...
            if self.plot_config:
                if np.mod(step_counter,100)==0: #plot every other 100 steps (100ms):
                    ...
                    plt.xticks([])
                    plt.yticks([])
                    plt.title('BVC_Nav_by_MemHpAz  t = ' + str(round(t,2)) + ' [s]')
                    mypause(0.002)
                    if self.vid_flag:
