...
class BVC_NAV:
    ...
    def __init__(self,Path,steer_thres,m_tau,telen_neurons,ang_bin,corals_pos,Goal_pos,agent_init_pos,df,eps,free_turn_bins,max_bins_turn,seed,sim_time,sim_num,plt,vid):
        ...
        self.sum_turns=0 #track turning directions
    ...
    def buildModel(self):
        ...
        def Steer(t,x):
            def _set_steer(t,DA):
                ...
                # self.DA=DA[ind] #tracking this variable
        ...
        def _get_potential_velocity(t, bvns_data):
            ...
            if 1:  # try to go desired direction
                ...
                if np.max(dist_des_HD)>D: #if path is clear
                    max_dir_ind=np.argmax(dist_des_HD) #choose the clearest path
                ...
                # # update desired direction backwards
                # if t > 1:
                #     self.sum_turns += self.turn_angle_CCW * self.dt
                #     if np.mod(t,2)<self.dt:  # don't update at the beginning, once in 2 sec
                #         v = Turn(v, -self.sum_turns)
                #         self.GoalDirection = calc_angle(t,v)  ##############################
                #         self.sum_turns=0 #restart counting
        ...
        self.sim = nengo.Simulator(self.model, dt=self.dt,  #1ms step size
                                       # optimize=False,
                                   seed=self.seed)
        ...
        while t < self.StopT+0.1:
            ...
            if self.plot_config:
                if np.mod(step_counter,100)==0: #plot every other 100 steps (100ms):
                    ...
                    plt.xticks([])
                    plt.yticks([])
                    plt.title('BVC_Nav_by_Azimuth t = ' + str(round(t,2)) + ' [s]')
                    # try:
                    #     plt.title('t=' + str(round(t, 2)) + ' [s]  DA: ' + str(self.DA) )
                    # except:
                    #     plt.title('t=' + str(round(t, 2)) + ' [s]')
                    mypause(0.002)
                    if self.vid_flag:
