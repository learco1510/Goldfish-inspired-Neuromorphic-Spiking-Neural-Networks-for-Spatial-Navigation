...
class BVC_NAV:
    ...
    def __init__(self,Path,arena_size,m_tau,RateScale,steer_thres,telen_neurons,ang_bin,corals_pos,Goal_pos,agent_init_pos,df,eps,free_turn_bins,max_bins_turn,seed,sim_time,sim_num,plt,vid):
        ...
        self.m_tau=m_tau
        self.goal_x_direction = 0  # default initialization
        ...
        self.goal_dist = arena_size[0] * (2**0.5) #default initial value- arena's diagonal size
        self.amp_gain = 1 / 100 * self.RS #gain for 'simple' neurons (so FR is in range 1-4)
        ...
        #arena parameters
        self.arena_size=arena_size #[4, 4] #arena size in [m]
        self.corals_pos = corals_pos
        self.n_corals=len(corals_pos)
        self.Goal_pos = Goal_pos #feeding position
        self.GoalDirection=float(np.arctan2( (Goal_pos[1]-self.init_pos[1]),
        ...
        self.pic_name = folder_path + "/SIM_" + str("%.3d"%sim_num)
    ...
    def buildModel(self):
        ...
        def calc_angle(t, x):
            return float(np.arctan2(x[1], x[0]) / self.max_angle)

        # speed and direction --> velocity
        def get_VxVy(t, x):
            return [x[1] * np.cos(x[0]), x[1] * np.sin(x[0])]

        # get array from 'rememberreefmap'
        def read_LIDAR(t, x):
            X, Y, HD = x
        ...
        def _get_potential_velocity(t, bvns_data):
            ...
            if 1:  # try to go desired direction
                ...
                # then, try to go desired direction
                ...
                )) # shift list to 0,n_LIDAR_pts-1 range
                ...
                # perform turn: smooth with previous turn
        ...
        def getGoalDirection(t,x):
            if t<.1: #at the beginning, keep only general direction to goal
                initial_x_dir_2goal=np.sign(Goal_pos[0]-self.init_pos[0]) #when reaching to Goal's HP, left or right?
                self.goal_x_direction = np.sign(initial_x_dir_2goal-1) #this shifts above sign=-1 to -1 (i.e -pi) and sign=1 to 0 (i.e. 0)
            elif abs(x)<(4*self.eps)*self.arena_size[1]/4: #if HP is as Goal's HP:
                self.GoalDirection=self.goal_x_direction
            else:
                self.GoalDirection= float(.5 * np.sign(x) * self.x_out)  #pi/2 if goal is shallower, -pi/2 if deeper, tuned
        ...
        self.model = spa.Network("BVC_NAV", seed=self.seed)
        with self.model:
            ...
            Agent_HD = nengo.Node(getGoalDirection,size_in=1,size_out=1)
            ...
            nHP = nengo.Ensemble(n_neurons=n_neurons,
                                  dimensions=1, radius=arena_size[1]/2,
                                  max_rates=HP_Rates * scale_rate,
                                  neuron_type=nengo.SpikingRectifiedLinear(amplitude=2 * self.RS / scale_rate)
                                  )
            nengo.Connection(npos[1],nHP,synapse=tau2)
            _add_probe(nHP, 'posHP')

            #Goal
            Goal = nengo.Node(Goal_pos)

            Goal_HP = nengo.Node(size_in=1,size_out=1) #Goal hydrostatic pressure
            nengo.Connection(Goal[1],Goal_HP[0])
            Goal_vertical_direction=nengo.Node(size_in=1,size_out=1) #gets the difference between goal and agent HP
            nengo.Connection(nHP,Goal_vertical_direction,transform=-1)  #previously was npos[1], not nHP
            nengo.Connection(Goal_HP,Goal_vertical_direction)
            nengo.Connection(Goal_vertical_direction,Agent_HD,synapse=tau2)
            ...
            ea_BVC = nengo.networks.EnsembleArray(neurons_per_angle,
                          ...
                          #     amplitude=1 / scale_rate)
                          neuron_type=
                                      # nengo.PoissonSpiking(
                              # nengo.RectifiedLinear(amplitude=1 / scale_rate)
                                nengo.SpikingRectifiedLinear(amplitude=1 * self.RS / scale_rate)
                                      # )
                          )
    ...
    def Start(self):
        ...
        while t < self.StopT+0.1:
            ...
            go2goal = self.sim.data[self.probes['Go2Goal']][-1]
            ...
            potHD = self.sim.data[self.probes['potHD']][-1:]
            ...
            if self.plot_config:
                if np.mod(step_counter,100)==0: #plot every other 100 steps (100ms):
                    ...
                    plt.xticks([])
                    plt.yticks([])
                    # plt.title('BVC_Nav_by_HydrostaticPressure   t = ' + str(round(t,2)) + ' [s]')
                    # plt.title('HP t = ' + str(round(t, 2)) + ' [s], GD= ' + str(round(self.GoalDirection,1)) +
                    #           ' GX= ' + str(round(self.goal_x_direction,1)))   #str(hd[0]-potHD[0]))
                    plt.title('HP t = ' + str(round(t, 2)) + ' [s], Goal= ' + str(go2goal)   )
                    # plt.title('HP t = ' + str(round(t, 2)) + ' [s], GD= ' + str(round(self.goal_dist,2))   )
